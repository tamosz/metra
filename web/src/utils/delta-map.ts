...
/** Build a lookup map from comparison deltas, keyed by className\0skillName\0tier\0scenario. */
...
export function deltaMapKey(className: string, skillName: string, tier: string, scenario: string): string {
  return `${className}\0${skillName}\0${tier}\0${scenario}`;
}
