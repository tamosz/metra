...
describe('class-colors', () => {
  describe('getClassColor', () => {
    it('returns correct color for known classes', () => {
      ...
      expect(getClassColor('Night Lord')).toBe('#55b8e0');
  ...
  describe('getClassColorWithOpacity', () => {
    ...
    it('handles opacity of 1', () => {
      const result = getClassColorWithOpacity('Night Lord', 1);
      // Night Lord = #55b8e0 → rgb(85, 184, 224)
      expect(result).toBe('rgba(85, 184, 224, 1)');
    });

    it('handles opacity of 0', () => {
      const result = getClassColorWithOpacity('Dark Knight', 0);
      // Dark Knight = #7855e0 → rgb(120, 85, 224)
